/// <summary>
/// mHpDotNetCtrl.cs - Interface for mHelp .NET Controls Management
/// mHelp version - 1.3
...
namespace mHpDotNetCtrl
{
    ...
    /// <summary>
    /// Class CmHpDotNetCtrl - Class to derive and implement in order to manage DotNET control
    /// </summary>
    public abstract class CmHpDotNetCtrl<DotNetCtrl>
    {
        // ************************************************
        // Properties
        ...
        /// <summary>
        /// GetDllVersion - Return DotNET control DLL version
        /// </summary>
        ...
        /// <summary>
        /// MoveUI - Position mHelp .NET control state config Form
        /// </summary>
        ...
        public bool MoveUI(Int32 hUI, int xPos, int yPos)
        {
            // Get control state config Form (using handle)
            Control CtrlUI = Control.FromHandle((IntPtr)hUI);
            if (CtrlUI == null)
                return false;
        ...
        protected abstract bool AddmHpEvent(Control Ctrl, string RegCtrlEvent);
        ...
        /// <summary>
        /// SetControl - Set standard DotNET control identifier (set 'CtrlId')
        /// </summary>
        /// <param name="Ctrl">Control object that emitted an event</param>
        protected void SetControl(Control Ctrl)
        {
            // Set standard DotNET control identifier (using 'Control.Name')
            CtrlId.Value = null;
            CtrlId.Value = new Object[1];
            CtrlId.Value[0] = Ctrl.Name;

            Control ParentCtrl = Ctrl;
            while (ParentCtrl.Parent != null)
            {
                ParentCtrl = ParentCtrl.Parent;
                if (ParentCtrl.Parent != null) // Avoid to add Form control
                {
                    Array.Resize(ref CtrlId.Value, CtrlId.Value.Length + 1);
                    CtrlId.Value[CtrlId.Value.Length - 1] = ParentCtrl.Name;
                }
            }
            Array.Reverse(CtrlId.Value);
        }

        private bool AddCtrlEvent(Control ParentCtrl, IntPtr hControl, string RegEvent)
        {
            // Loop on all control defined in the Form
            foreach (Control ChildCtrl in ParentCtrl.Controls)
            {
                bool bCtrlFound = false;

                // Check if child control match with control handle
                if (ChildCtrl.Handle.ToInt32() == hControl.ToInt32())
                    bCtrlFound = true;
                else
                {
                    // Check if child control match with the 16 lower bits control handle
                    int ChildCtrlHandle = ChildCtrl.Handle.ToInt32();
                    ChildCtrlHandle &= 0x0000FFFF;
                    if (ChildCtrlHandle == hControl.ToInt32())
                        bCtrlFound = true;
                }

                // Check if control handle match with child control
                if (bCtrlFound)
                {
                    // Check if control match with expected DotNET control object
                    if (ChildCtrl.GetType() != typeof(DotNetCtrl))
                        return true; // No error (not managed with this DLL)

                    ////// Add mHelp event handler
                    return AddmHpEvent(ChildCtrl, RegEvent);
                }

                // Check if child control has children
                if (ChildCtrl.HasChildren)
                    AddCtrlEvent(ChildCtrl, hControl, RegEvent); // Recursive
            }
        ...
        /// </summary>
        /// <param name="hForm">Form handle (containing control)</param>
        /// <param name="hControl">Control handle</param>
        /// <returns>TRUE: Event(s) added successfully; FALSE: Otherwise</returns>
        public bool AddmHpEvents(Int32 hForm, Int32 hControl)
        {
            // Get Form from handle
            Control ParentCtrl = Control.FromHandle((IntPtr)hForm);
            if (ParentCtrl == null)
                return true; // Not a DotNET form
            ...
            foreach (string CurRegEvent in CtrlEvent)
            {
                ...
                if (!AddCtrlEvent(ParentCtrl, (IntPtr)hControl, CurRegEvent))
            ...
            // - Not an expected DotNET control object (not managed with this DLL)
            // - Control not found (control handle did not match with any DotNET control)
            return true;
        }

        /// <summary>
        /// RegistermHpEvent - Register a new window message to inform mHelp a DotNET control event has occurred
        /// </summary>
        ...
        ///                mHelp a managed DotNET control event has occurred
        /// </summary>
        ...
        protected bool SendmHpEvent(Object CtrlSender, string RegCtrlEvent)
        {
            ...
            uint imHpDotNetMsg = (uint)EventRegTab[RegCtrlEvent];

            // Get parent Form
            Control FormCtrl = CtrlSender as Control;
            while (FormCtrl.Parent != null)
                FormCtrl = FormCtrl.Parent;

            // Send message to inform mHelp a DotNET control event has occured
            mHpCtrlConst.SendMessage(FormCtrl.Handle, imHpDotNetMsg, IntPtr.Zero, IntPtr.Zero);
        ...
        public bool CaptureEvent(uint imHpDotNetMsg)
        {
            // Check if message match with a registered event (window message)
            return EventRegTab.ContainsValue(imHpDotNetMsg);
        }
        ...
        private Control GetChildControl(Control ParentCtrl, ref uint iIdIdx)
        {
            // Loop on controls defined in the parent control
            foreach (Control ChildCtrl in ParentCtrl.Controls)
            {
                ...
                // Check if child control name match with current control identifier
                if (ChildCtrl.Name == (string)CtrlId.Value[iIdIdx])
                {
                    ...
                    {
                        // Check if control match with expected DotNET control object type
                        if (ChildCtrl.GetType() != typeof(DotNetCtrl))
                ...
                if (ChildCtrl.HasChildren)
                {
                    // Get control from child control
                    Control InChildCtrl = GetChildControl(ChildCtrl, ref iIdIdx);
                    if (InChildCtrl != null)
                    {
                        // Check if control match with expected DotNET control object type
                        if (InChildCtrl.GetType() != typeof(DotNetCtrl))
                            return null; // Not the expected control object type found
        ...
        /// <summary>
        /// GetControl - Get standard DotNET control object using parent Form handle and standard control
        ///              identifier stored in 'CtrlId' (set 'CtrlObj')
        /// </summary>
        /// <param name="hForm">Parent Form handle</param>
        /// <returns>TRUE: 'CtrlObj' control object set successfully; FALSE: Otherwise</returns>
        protected bool GetControl(IntPtr hForm)
        {
            // Get Form from handle
            Control ParentCtrl = Control.FromHandle(hForm);
                ...
                (CtrlId.Value[0].GetType() == typeof(string)))
            {
                ...
                uint iIdIdx = 0;
                CtrlObj = (Object)GetChildControl(ParentCtrl, ref iIdIdx);
                if (CtrlObj != null)
        ...
        public abstract bool Start(Int32 hForm);
        ...
        /// <summary>
        /// CmHpDotNetCtrl - Constructor
        /// </summary>
        public CmHpDotNetCtrl()
        {
            // Initialization
            CtrlStyle = mHpCtrlConst.NO_VALUE;
