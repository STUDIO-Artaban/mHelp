...
bool mHpWebClient::ole_is_initialized_ = false;

const IID IID_IDocHostUIHandler ={0xbd3f23c0,0xd43e,0x11CF,{0x89, 0x3b, 0x00, 0xaa, 0x00, 0xbd, 0xce, 0x1a}};


mHpWebClient::mHpWebClient(HINSTANCE instance) : 
  instance_(instance), browserObject_(0), bUserTrigURL_(false)
{
  TraceFunc("mHpWebClient::mHpWebClient");

  Create(0, 0, 200, 200, HWND_DESKTOP);
}

mHpWebClient::mHpWebClient(
...
HRESULT mHpWebClient::QueryInterface(REFIID riid, void ** ppvObject) {
  TraceFunc("mHpWebClient::QueryInterface");
...
long mHpWebClient::CreateBrowserObject() {
  TraceFunc("mHpWebClient::CreateBrowserObject");
  RECT             rect;
...
void mHpWebClient::Resize(DWORD width, DWORD height) {
  IWebBrowser2  *webBrowser2;
...
void mHpWebClient::Size(int width, int height) {
  RECT r;
...
long mHpWebClient::URL(std::string const& url) {
  IWebBrowser2  *webBrowser2;
  VARIANT         vURL;
  ::VariantInit(&vURL);
  vURL.vt = VT_BSTR;

  TraceFunc("mHpWebClient::URL");
